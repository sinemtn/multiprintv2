...
type TugasMultiDeleteProps<TData> = {
    open: boolean
...
export function TugasMultiDelete<TData>({
  open,
  onOpenChange,
  table,
}: TugasMultiDeleteProps<TData>) {
    ...
    toast.promise(sleep(2000), {
      ...
      success: () => {
        ...
        return `Deleted ${selectedRows.length} ${
          selectedRows.length > 1 ? 'Tugas' : 'Tugas'
        }`
      },
      error: 'Error',
    })
    ...
    <ConfirmDialog
      ...
      title={
        <span className='text-destructive'>
          ...
          {selectedRows.length > 1 ? 'Printers' : 'Printer'}
        </span>
      }
      desc={
