...
const route = getRouteApi('/_authenticated/stock/toner/')
type DataTableProps = {
    data: StockToner[]
}
...
export function TasksTable({ data }: DataTableProps) {
    ...
    const table = useReactTable({
        ...
        globalFilterFn: (row, _columnId, filterValue) => {
            ...
            const qty = String(row.getValue('qty')).toLowerCase()
            const customer = String(row.getValue('customer')).toLowerCase()
            const searchValue = String(filterValue).toLowerCase()
            return id.includes(searchValue) || location.includes(searchValue) || branch.includes(searchValue) || qty.includes(searchValue) || customer.includes(searchValue)
        },
